...
function getEvents(artist) {
  $.ajax({
    type: 'GET',
    url:
      'https://app.ticketmaster.com/discovery/v2/events.json?size=1&apikey=jqljeDC6qh4bmsy6Ebo3TuTS5YGMzOaR&keyword=' +
      artist,
    async: true,
    dataType: 'json',
        ...
        json._embedded.events.forEach(event => {
          ...
          if (concert.dates.status.code === 'onsale') {
            $('#tourSale').html(`<div>Tickets are still available</div>
                                    <a target=”_blank” href=${concert.url}>Buy Tickets Here</a>`)
            if (event.priceRanges) {
              if (event.priceRanges[0].min === event.priceRanges[0].max) {
                $('#tourSale').append(`<div>Price: $${event.priceRanges[0].max}</div>`)
              } else {
                $('#tourSale').append(
                  `<div>Price Range: $${event.priceRanges[0].min} - $${event.priceRanges[0].max}</div>`
                )
              }
            }
          }
